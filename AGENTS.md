...
- Stripe Accounts v2 helpers live under `src/stripe/` and are exposed as `node dist/index.js stripe …` (create-account, create-account-link, create-checkout-session, create-product, create-setup-intent, create-subscription, handle-webhook). Set `STRIPE_SECRET_KEY` from the Stripe Dashboard; leave the Stripe SDK API version unset. Local merchant / Stripe IDs persist in `~/.cubic-plugin/stripe-store.json` (override with `STRIPE_DATA_PATH`).
