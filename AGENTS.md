...
- Stripe Accounts v2 integration lives under `src/stripe/` and is exposed as `cubic-plugin stripe …` domain commands (`create-account`, `create-account-link`, `create-checkout-session`, `create-subscription-plan`, `attach-balance-payment-method`, `create-subscription`, `handle-webhooks`, plus Workbench-style `inspect` / `api` / `update`). Copy `.env.example` to `.env` and set `STRIPE_SECRET_KEY` / `STRIPE_PUBLISHABLE_KEY` from the Stripe Dashboard. Resource ids are persisted in `.cubic-stripe.json` (override with `CUBIC_STRIPE_STORE`). The Stripe client is initialized without a hard-coded API version. `stripe inspect <id>` retrieves an object, related-object data map, and matching Events (request logs are Dashboard Workbench-only; the CLI links to them). `stripe api` / `stripe update` mirror Shell + API Explorer edits and block live-mode mutations by default.
