...
- Stripe Accounts v2 integration lives under `src/stripe/` and is exposed as `cubic-plugin stripe …` domain commands (`create-account`, `create-account-link`, `create-checkout-session`, `create-subscription-plan`, `attach-balance-payment-method`, `create-subscription`, `handle-webhooks`, `show-status`, plus Workbench-style `inspect` / `update` / `run-request` under `src/stripe/workbench/`). Copy `.env.example` to `.env` and set `STRIPE_SECRET_KEY` / `STRIPE_PUBLISHABLE_KEY` from the Stripe Dashboard. Resource ids are persisted in `.cubic-stripe.json` (override with `CUBIC_STRIPE_STORE`). The Stripe client is initialized without a hard-coded API version. Test mode is detected when the secret key contains `_test_`; mutating Workbench commands are blocked in live mode unless `--allow-live` is passed.
