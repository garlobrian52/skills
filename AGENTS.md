...
- Stripe integration lives under `src/stripe/` and is exposed as `cubic-plugin stripe …` domain commands (`create-product`, `create-checkout-session`, `create-account`, `create-account-link`, `create-subscription-plan`, `attach-balance-payment-method`, `create-subscription`, `handle-webhooks`). One-time Checkout flow: `create-product` then `create-checkout-session` (reuses catalog `priceId`), then `handle-webhooks` for `checkout.session.completed`. Copy `.env.example` to `.env` and set `STRIPE_SECRET_KEY` / `STRIPE_PUBLISHABLE_KEY` from the Stripe Dashboard. Resource ids are persisted in `.cubic-stripe.json` (override with `CUBIC_STRIPE_STORE`). The Stripe client is initialized without a hard-coded API version.
