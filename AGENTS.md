...
- Stripe Accounts v2 integration lives under `src/stripe/` and is exposed as `cubic-plugin stripe …` domain commands (`create-account`, `create-account-link`, `create-checkout-session`, `create-subscription-plan`, `attach-balance-payment-method`, `create-subscription`, `inspect-object`, `api-request`, `handle-webhooks`). Copy `.env.example` to `.env` and set `STRIPE_SECRET_KEY` / `STRIPE_PUBLISHABLE_KEY` from the Stripe Dashboard. Resource ids are persisted in `.cubic-stripe.json` (override with `CUBIC_STRIPE_STORE`). The Stripe client is initialized without a hard-coded API version. `inspect-object` mirrors Workbench Inspector (object JSON, related data map, events, request summaries); `api-request` mirrors Shell / API Explorer for GET/POST/DELETE edits.
