...
- CLI telemetry uses `posthog-js` (lazy init in `src/posthog.ts`) and is enabled by default with a bundled public project key. A non-empty `POSTHOG_API_KEY` overrides that key; set `POSTHOG_API_KEY=""` to disable telemetry. `npm test` does this automatically so the suite does not send events.
