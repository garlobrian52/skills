...
- Full installs write OAuth-ready MCP config (no `CUBIC_API_KEY` / `Authorization` headers). Users authenticate later through their coding tool's MCP login flow. `--skills-only` skips MCP config entirely. `--json` mode is non-interactive and does not require an API key.
- Default installs target each tool's home-directory config path under `~`. Already-installed targets are skipped unless `--force` is passed. Use `--output` to write into an isolated directory for smoke tests.
